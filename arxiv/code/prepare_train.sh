...
for pair in en-de lv-en; do
    ...
    [[ ! -d data/$pair ]] && mkdir data/$pair
    cd data/$pair

    # Tokenize, normalize
    for lang in $src $tgt; do
        for prefix in $(cat train.txt); do
	          cat $prefix.$lang
        done | $MOSES/scripts/tokenizer/normalize-punctuation.perl -l $lang \
            | $MOSES/scripts/tokenizer/remove-non-printing-char.perl \
            | $MOSES/scripts/tokenizer/tokenizer.perl -q -no-escape -protected $(dirname $0)/../tokenizer/basic-protected-patterns -l $lang > train.tok.$lang &
    done
