...
if len(sys.argv[1:]) >= 4:
    ...
    in_src_fname, in_trg_fname, out_src_fname, out_trg_fname = \
        'train.src.bpe', 'train.trg.bpe', 'train.src.bpe.filter', 'train.trg.bpe.filter'

max_tokens = int(sys.argv[5]) if len(sys.argv[1:]) > 4 else 95

with open(in_src_fname, mode='rt', encoding='utf-8') as in_src, \
     open(in_trg_fname, mode='rt', encoding='utf-8') as in_trg, \
     open(out_src_fname, mode='wt', encoding='utf-8') as out_src, \
     open(out_trg_fname, mode='wt', encoding='utf-8') as out_trg:
