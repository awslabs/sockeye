...
- Sockeye can now train models that leverage sentence-level metadata in the form of name-weight pairs using the method described by Schioppa et al. ([2021](https://aclanthology.org/2021.emnlp-main.535)).
  ...
  - To train a metadata-aware model, use `sockeye-train` argument `--encoder-add-metadata`. For a given sentence, the model maps each name in the metadata dictionary to an embedding, multiplies it by the corresponding weight, and sums the results for a sentence-level embedding. The sentence-level embedding is then added to the encoder representation for each position at the specified layer or layers. `last`: learn a single set of embedding weights and add embeddings after the last encoder layer. `all_tied`: learn a single set of embedding weights and add embeddings after all encoder layers. `all`: learn a separate set of embedding weights for each encoder layer and add them after each corresponding layer.
  - To add metadata support to a non-metadata model via fine-tuning, prepare an adaptation set with metadata and add the `--encoder-add-metadata` argument when running `sockeye-train --params ...`.
  - To fine-tune a metadata model on a non-metadata data set, copy the model's metadata vocabulary (`vocab.md.json`) to the prepared data directory for the adaptation set before running `sockeye-train --params ...`.
