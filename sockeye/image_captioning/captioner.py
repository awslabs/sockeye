...
from sockeye.image_captioning import utils
from sockeye.image_captioning.extract_features import get_pretrained_net, \
...
def get_pretrained_caption_net(args: argparse.Namespace,
                               context: mx.Context,
                               image_preextracted_features: bool) -> inference.ImageCaptioner:
...
def _extract_features(args, context):
    ...
    with open(output_file, "w") as fout:
        for i, im in enumerate(batching(image_list, args.batch_size)):
            feats, out_names = extract_features_forward(im, image_model, args.source_root,
                                                        output_root, args.batch_size,
                                                        args.source_image_size, context)
            # Save to disk
...
def main():
    ...
    with ExitStack() as exit_stack:
        ...
        translator = get_pretrained_caption_net(args, context, image_preextracted_features)
