...
class InferenceModel(model.SockeyeModel):
    ...
    def _get_encoder_module(self) -> Tuple[mx.mod.BucketingModule, int]:
        ...
        def sym_gen(source_seq_len: int):
            ...
            source_length = utils.compute_lengths(source, dtype=np.int32)
...
def load_models(context: mx.context.Context,
                ...
                cache_output_layer_w_b: bool = False,
                encoder_dtype=None,
                decoder_dtype=None) -> Tuple[List[InferenceModel], Dict[str, int], Dict[str, int]]:
    """
...
class Translator:
    ...
    def _get_inference_input(self, sequences: List[List[str]]) -> Tuple[mx.nd.NDArray, int]:
        ...
        source = mx.nd.zeros((len(sequences), bucket_key), dtype=np.int32)
