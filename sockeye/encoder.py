...
class AddMetadataEmbeddings(Encoder):
    """
    Module that generates sequence-level metadata embeddings and adds them to
    existing model states using the method described by Schioppa et al. (2021,
    https://aclanthology.org/2021.emnlp-main.535/).

    Metadata embeddings are generated by lookup up embeddings for metadata IDs,
    multiplying them by the corresponding weights, and summing in the sequence
    dimension. The resulting sequence-level embeddings are added to the existing
    model states with broadcasting in the sequence dimension. Model state and
    metadata sequence lengths are independent.

    This module uses shape information. For correct results, script it before
    tracing anything that includes it as a submodule:
    `add_metadata_embeddings = torch.jit.script(AddMetadataEmbeddings(...))`

    :param vocab_size: Input vocabulary size.
    :param model_size: Model state size.
    ...
    def forward(self, data: pt.Tensor, name_ids: pt.Tensor, weights: pt.Tensor) -> pt.Tensor:
        """
        Generate sequence-level metadata embeddings and add them to existing
        model states.

        :param data: Existing model states.
                     Shape: (batch_size, states_seq_len, model_size).
        ...
        :returns: Model states with sequence-level metadata embeddings added.
                  Shape: (batch_size, seq_len, model_size).
        """
        if name_ids.numel() == 0:
            # No metadata
            return data
        ...
        summed_embeddings = pt.sum(weighted_embeddings, dim=1).unsqueeze(1)

        if self.dropout is not None:
            summed_embeddings = self.dropout(summed_embeddings)

        # (batch_size, states_seq_len, model_size)
        return data + summed_embeddings
